...
flags.DEFINE_bool("use_image_obs", True, "Whether to include image observations.")
flags.DEFINE_bool("fixed_start_poses", False, "Whether to use fixed start " "poses.")
flags.DEFINE_bool(
    "noisy_ee_pose",
    False,
    "Whether to use noisy pose "
    "for end effector so it does not start in exact position.",
)
flags.DEFINE_bool(
    "no_episode_step_limit", False, "If True, remove max_episode_steps step limit."
)
flags.DEFINE_string(
    "tfagents_path",
    None,
    "If set a dataset of the oracle output will be saved " "to the given path.",
)
...
flags.DEFINE_bool(
    "save_successes_only", True, "Whether to save only successful episodes."
)
flags.DEFINE_integer("num_episodes", 1, "The number of episodes to collect.")
...
def main(argv):
    ...
    if FLAGS.video:
        ...
        env = mp4_video_wrapper.Mp4VideoWrapper(
            env,
            env.control_frequency,
            frame_interval=1,
            video_filepath=FLAGS.video_path,
        )
    ...
    while True:
        ...
        if hasattr(env, "instruction") and env.instruction is not None:
            logging.info(
                "Current instruction: %s", env.decode_instruction(env.instruction)
            )
        ...
        while not done:
            action = oracle_policy.action(
                time_step, oracle_policy.get_initial_state(1)
            ).action
        ...
        if done:  # episode terminated normally (not on manual reset w.o. saving).
            ...
            if observers:
                ...
                logging.info(
                    "Recording %d length episode to shard %d",
                    len(episode_data.action),
                    cur_observer,
                )
            ...
            if FLAGS.pybullet_state_path is not None:
                serialize_module.write_pybullet_data(
                    FLAGS.task,
