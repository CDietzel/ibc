...
EpisodeData = collections.namedtuple(
    "EpisodeData", ("time_step", "action", "pybullet_state")
)


def get_tfrecord_observers(
    env, tfagents_path=None, worker_id=None, dataset_nshards=None
):
...
def _init_observers(env, tfagents_path, worker_id, dataset_nshards):
    ...
    for i in range(dataset_nshards):
        ...
        data_spec = trajectory.from_transition(
            env.time_step_spec(),
            policy_step.PolicyStep(env.action_spec()),
            env.time_step_spec(),
        )
        observers.append(
            example_encoding_dataset.TFRecordObserver(
                dataset_split_path[0]
                + "_%d" % i
                + "_worker-%d" % worker_id
...
def write_tfagents_data(episode_data, observer):
...
def write_pybullet_data(task, pybullet_state_path, episode_data, i_episode, worker_id):
    """Write out the pybullet json for replay."""
    actions = [policy_step.action.tolist() for policy_step in episode_data.action]
    filename = "%s_ep%d_worker-%d.json.zip" % (
        pybullet_state_path,
        i_episode,
        worker_id,
    )
    write_pybullet_state(filename, episode_data.pybullet_state, task, actions=actions)
