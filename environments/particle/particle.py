...
class ParticleEnv(gym.Env):
    ...
    * = set in the Oracle params, not the env.
    ...
    def get_metrics(self, num_episodes):
        metrics = [
            particle_metrics.AverageFirstGoalDistance(self, buffer_size=num_episodes),
            particle_metrics.AverageSecondGoalDistance(self, buffer_size=num_episodes),
            particle_metrics.AverageFinalSecondGoalDistance(
                self, buffer_size=num_episodes
            ),
            particle_metrics.AverageSuccessMetric(self, buffer_size=num_episodes),
        ]
    ...
    def _create_observation_space(self):
        obs_dict = collections.OrderedDict(
            pos_agent=spaces.Box(
                low=0.0,
                high=1.0,
                shape=(self.n_dim,),  # pytype: disable=attribute-error
                dtype=np.float32,
            ),
            # TODO(peteflorence): is this the actual max for vel_agent?
            vel_agent=spaces.Box(
                low=-1e2,
                high=1e2,
                shape=(self.n_dim,),  # pytype: disable=attribute-error
                dtype=np.float32,
            ),
            pos_first_goal=spaces.Box(
                low=0.0,
                high=1.0,
                shape=(self.n_dim,),  # pytype: disable=attribute-error
                dtype=np.float32,
            ),
            pos_second_goal=spaces.Box(
                low=0.0,
                high=1.0,
                shape=(self.n_dim,),  # pytype: disable=attribute-error
                dtype=np.float32,
    ...
    def reset(self):
        ...
        obs = dict()
        obs["pos_agent"] = self._rng.rand(self.n_dim).astype(
            np.float32
        )  # pytype: disable=attribute-error
        obs["vel_agent"] = np.zeros((self.n_dim)).astype(
            np.float32
        )  # pytype: disable=attribute-error
        obs["pos_first_goal"] = self._rng.rand(self.n_dim).astype(
            np.float32
        )  # pytype: disable=attribute-error
        obs["pos_second_goal"] = self._rng.rand(self.n_dim).astype(
            np.float32
        )  # pytype: disable=attribute-error
    ...
    def _internal_step(self, action, new_action):
        ...
        u_agent = self.k_p * (
            action - obs["pos_agent"]
        ) - self.k_v * (  # pytype: disable=attribute-error
            obs["vel_agent"]
        )
        new_xy_agent = (
            obs["pos_agent"] + obs["vel_agent"] * self.dt
        )  # pytype: disable=attribute-error
        new_velocity_agent = (
            obs["vel_agent"] + u_agent * self.dt
        )  # pytype: disable=attribute-error
    ...
    def dist(self, goal):
        current_position = self.obs_log[-1][
            "pos_agent"
        ]  # pytype: disable=attribute-error
    ...
    def _get_reward(self, done):
        ...
        self.min_dist_to_first_goal = min(
            self.dist(
                self.obs_log[0]["pos_first_goal"]
            ),  # pytype: disable=attribute-error
            self.min_dist_to_first_goal,
        )  # pytype: disable=attribute-error
        self.min_dist_to_second_goal = min(
            self.dist(
                self.obs_log[0]["pos_second_goal"]
            ),  # pytype: disable=attribute-error
    ...
    def step(self, action):
        ...
        state = self._get_state()
            ...
            True if self.steps >= self.n_steps else False
        )  # pytype: disable=attribute-error
    ...
    def render(self, mode="rgb_array"):
        ...
        if self.n_dim == 2:  # pytype: disable=attribute-error
            fig, _ = particle_viz.visualize_2d(
                self.obs_log, self.act_log
            )  # pytype: disable=attribute-error
        else:
            fig, _ = particle_viz.visualize_nd(
                self.obs_log, self.act_log
            )  # pytype: disable=attribute-error
        fig.canvas.draw()
        ...
        if traj.is_last():
            ...
            if self.n_dim == 2:  # pytype: disable=attribute-error
                ...
                filename = os.path.join(
                    self.img_save_dir,  # pytype: disable=attribute-error
