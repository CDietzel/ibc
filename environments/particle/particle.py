    ...
    * = set in the Oracle params, not the env.
    ...
    def _create_observation_space(self):
        obs_dict = collections.OrderedDict(
            ...
            vel_agent=spaces.Box(
                low=-1e2, high=1e2, shape=(self.n_dim,), dtype=np.float32
            ),
    ...
    def _get_reward(self, done):
        ...
        self.min_dist_to_first_goal = min(
            self.dist(self.obs_log[0]["pos_first_goal"]), self.min_dist_to_first_goal
        )
        self.min_dist_to_second_goal = min(
            self.dist(self.obs_log[0]["pos_second_goal"]), self.min_dist_to_second_goal
        )
        ...
        if traj.is_last():
            ...
            if self.n_dim == 2:
                ...
                filename = os.path.join(
                    self.img_save_dir, str(self.reset_counter).zfill(6) + "_2d.png"
                )
                fig.savefig(filename)
                plt.close(fig)
            fig, _ = particle_viz.visualize_nd(self.obs_log, self.act_log)
            filename = os.path.join(
                self.img_save_dir, str(self.reset_counter).zfill(6) + "_nd.png"
            )
            fig.savefig(filename)
            plt.close(fig)
