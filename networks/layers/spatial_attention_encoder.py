...
class SpatialAttentionEncoder(tf.keras.layers.Layer):
    ...
    def __init__(
        self, filters, num_blocks=8, num_heads=64, target_height=90, target_width=120
    ):
    ...
    def _concat_coords(self, obs, batch_size):
        image_coords = tf.broadcast_to(
            self.coords, (batch_size, self.target_height, self.target_width, 2)
        )
...
def conv(filters, kernel_size):
    return tf.keras.layers.Conv2D(filters, kernel_size, padding="same", activation=None)
...
class SpatialSoftmax(tf.keras.layers.Layer):
    ...
    def __init__(self):
        ...
        self.temperature = tf.Variable(
            initial_value=1.0, trainable=True, name="spatial_softmax_temperature"
        )
