...
class ConvMLPMDN(network.Network):
    ...
    def _init_coord_conv(self):
        posy, posx = tf.meshgrid(
            tf.linspace(-1.0, 1.0, num=self.target_height),
            tf.linspace(-1.0, 1.0, num=self.target_width),
            indexing="ij",
        )
    ...
    def _concat_coordconv(self, obs, batch_size):
        image_coords = tf.broadcast_to(
            self.image_coords, (batch_size, self.target_height, self.target_width, 2)
        )
    ...
    def call(self, inputs, training, step_type=(), network_state=()):
        ...
        x = tfd.MixtureSameFamily(tfd.Categorical(logits=pi), components_distribution)
