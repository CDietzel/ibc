...
def cd(predictions):
    ...
    per_example_loss = tf.reduce_mean(energy_samp, axis=1) - tf.reduce_mean(
        energy_data, axis=1
    )
    return per_example_loss, dict()
...
def cd_kl(predictions, counter_example_actions, predictions_copy):
    ...
    energy_data = predictions[:, -1:]
    energy_samp = predictions[:, :-1]
    cd_per_example_loss = tf.reduce_mean(energy_samp, axis=1) - tf.reduce_mean(
        energy_data, axis=1
    )
    debug_dict = dict()
    ...
    kl_per_example_loss = tf.reduce_mean(
        -energy_samp_copy[Ellipsis, None] - entropy, axis=[-2, -1]
    )
...
def clipped_cd(predictions, counter_example_actions, actions_size_n, soft):
    ...
    outside_thresh = tf.where(
        errors > thresh, tf.ones_like(errors), tf.zeros_like(errors)
    )
