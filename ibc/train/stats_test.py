...
class StatsTest(tf.test.TestCase):
    ...
    def test_norm_denorm(self):
        ...
        (_, act_norm_layers, act_denorm_layer, _, _) = stats.compute_dataset_statistics(
    ...
    def test_flat_stats(self):
        ...
        np.testing.assert_almost_equal(obs_mean, obs_norm_layers._mean, decimal=3)
        np.testing.assert_almost_equal(obs_std, obs_norm_layers._std, decimal=3)
        ...
        np.testing.assert_almost_equal(act_mean, act_norm_layers._mean, decimal=3)
        np.testing.assert_almost_equal(act_std, act_norm_layers._std, decimal=3)
    ...
    def test_nested_obs_stats(self):
        ...
        for observation, obs_norm_layer in zip(
            ...
            np.testing.assert_almost_equal(
                obs_mean, obs_norm_layer.layers[0]._mean, decimal=3
            )
            np.testing.assert_almost_equal(
                obs_std, obs_norm_layer.layers[0]._std, decimal=3
            )
    ...
    def test_nested_img_stats(self):
        ...
        (
            ...
            dataset, num_samples=NUM_SAMPLES, nested_obs=True, nested_actions=True
        )
        ...
        for observation, obs_norm_layer in zip(
            ...
            observation = np.reshape(observation.numpy(), [-1, observation.shape[-1]])
            obs_mean = np.mean(observation, axis=0)
            obs_std = np.std(observation, axis=0)
        ...
        np.testing.assert_almost_equal(
            act_mean, act_norm_layers._nested_layers.layers[0]._mean, decimal=3
        )
        np.testing.assert_almost_equal(
            act_std, act_norm_layers._nested_layers.layers[0]._std, decimal=3
        )
        np.testing.assert_almost_equal(act_min, min_actions)
        np.testing.assert_almost_equal(act_max, max_actions)
