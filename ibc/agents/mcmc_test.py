...
class McmcTest(tf.test.TestCase):
    def _test_batched_categorical_bincount_shapes(self, batch_size):
        ...
        indices_counts = mcmc.categorical_bincount(
            count, tf.math.log(probs), num_samples
        )
    ...
    def test_batched_categorical_bincount_correct(self):
        eps = 1e-6
        probs = np.array([[eps, eps, 1.0 - eps], [eps, 1.0 - eps, eps]])
        count = 5
        indices_counts = mcmc.categorical_bincount(
            count, tf.math.log(probs), probs.shape[1]
        ).numpy()
    ...
    def _get_network_and_time_step(self):
        ...
        energy_network = mlp_ebm.MLPEBM((obs_spec, act_spec), tf.TensorSpec([1]))
        ...
        class EnergyNet(tf.keras.Model):
            ...
            def call(self, x, step_type=(), network_state=(), training=()):
                ...
                return (
                    -(
                        (
                            tf.linalg.norm(actions - self.mean, axis=1)
                            * self.energy_scale
                        )
                        ** 2
                    ),
    ...
    def test_shapes_iterative_dfo(self):
        ...
        for key in obs.keys():
            ...
            obs[key] = tf.concat(
                [batch_obs] * (batch_size * num_action_samples), axis=0
            )

        init_action_samples = np.random.rand(batch_size * num_action_samples, 2).astype(
            np.float32
        )
        ...
        probs, action_samples, _ = mcmc.iterative_dfo(
            ...
            min_actions=np.array([0.0, 0.0]).astype(np.float32),
            max_actions=np.array([1.0, 1.0]).astype(np.float32),
            num_iterations=3,
            iteration_std=1e-1,
            training=False,
            tfa_step_type=(),
    ...
    def test_correct_iterative_dfo(self):
        ...
        assert (
            tf.linalg.norm(action_samples[np.argmax(probs)] - energy_network.mean)
            < 0.01
        )
    ...
    def test_correct_langevin(self):
        ...
        assert tf.linalg.norm(action_samples[0] - energy_network.mean) < 0.1
