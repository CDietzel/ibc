...
class ImplicitBCAgent(base_agent.BehavioralCloningAgent):
    ...
    def _loss(self, experience, variables_to_train=None, weights=None, training=False):
        ...
        if self._late_fusion:
            ...
            maybe_tiled_obs = nest_utils.tile_batch(
                observations, self._num_counter_examples + 1
            )
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            ...
            with tf.name_scope("loss"):
                ...
                if self._compute_mse:
                    ...
                    mse_counter_examples = self._mse(
                        counter_example_actions, actions_size_n
                    )
                ...
                if self._late_fusion:
                    ...
                    obs_embeddings = self.cloning_network.encode(
                        maybe_tiled_obs, training=True
                    )
                ...
                predictions = tf.reshape(
                    predictions, [batch_size, self._num_counter_examples + 1]
                )
    ...
    def _make_counter_example_actions(
        ...
        if self._fraction_dfo_samples == 0.0 and self._fraction_langevin_samples == 0.0:
            ...
            if self._late_fusion:
                ...
                maybe_tiled_obs_n = nest_utils.tile_batch(
                    observations, self._num_counter_examples
                )
            ...
            dfo_num_indices = int(
                self._fraction_dfo_samples * self._num_counter_examples
            )
            langevin_num_indices = int(
                self._fraction_langevin_samples * self._num_counter_examples
            )
            residual = (
                self._num_counter_examples
                - init_num_indices
