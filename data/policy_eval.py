...
flags.DEFINE_integer(
    "history_length", None, "If set the previous n observations are stacked."
)
flags.DEFINE_bool("video", False, "If true record a video of the evaluations.")
flags.DEFINE_bool(
    "viz_img", False, "If true records an img of evaluation trajectories."
)
flags.DEFINE_string("output_path", "/tmp/ibc/policy_eval/", "Path to save videos at.")
flags.DEFINE_enum(
    "task",
    None,
    [
...
flags.DEFINE_bool("use_image_obs", False, "Whether to include image observations.")
flags.DEFINE_bool(
    "flatten_env", False, "If True the environment observations are flattened."
)
flags.DEFINE_bool(
    "shared_memory", False, "If True the connection to pybullet uses shared memory."
)
flags.DEFINE_string("saved_model_path", None, "Path to the saved_model policy to eval.")
flags.DEFINE_string("checkpoint_path", None, "Path to the checkpoint to evaluate.")
flags.DEFINE_enum(
    "policy",
    None,
    [
        ...
        "oracle_reach",
        "oracle_push",
        "oracle_reach_normalized",
        "oracle_push_normalized",
        "particle_green_then_blue",
    ],
...
flags.DEFINE_string(
    "dataset_path",
    None,
    "If set a dataset of the policy evaluation will be saved " "to the given path.",
)
flags.DEFINE_integer(
    ...
    if video:
        ...
        if saved_model_path:
            ...
            video_path = os.path.join(
                video_path, policy_name + "_" + checkpoint_ref + "vid.mp4"
            )
...
def main(_):
    ...
    gin.parse_config_files_and_bindings(flags.FLAGS.gin_file, flags.FLAGS.gin_bindings)
