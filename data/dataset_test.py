...
from ibc.environments.block_pushing.oracles import (
    oriented_push_oracle as oriented_push_oracle_module,
)
import numpy as np
...
class LoadTFRecordDatasetSequence(tf.test.TestCase):
    def _get_test_step(self, global_step, step_type, dataset_spec):
    ...
    def _init_test_shards(self, num_shards, episodes_per_shard, steps_per_episode):
    ...
    def _check_sample(self, sample, expected_values, step_type, next_step_type):
        ...
        for value in sample.observation.values():
            ...
            self.assertEqual(
                value.numpy().tolist(), [[v] * value.shape[1] for v in expected_values]
            )

    def test_load_tfrecord_dataset_sequence(self):
        shards = self._init_test_shards(
            num_shards=2, episodes_per_shard=2, steps_per_episode=4
        )
        dataset = load_tfrecord_dataset_sequence(shards, seq_len=3, deterministic=True)
